...
<script type="text/javascript">
  ...
  function delete_form(list_by_news_no) {
    $('#btn_delete').attr('data-content', list_by_news_no); // 삭제할 파일 번호 임시 저장
  ...
  function delete_proc() {
    var list_by_news_no = $('#btn_delete').attr('data-content'); // 삭제할 파일 번호 임시 저장
    var frm = $('#frm')
    $('#list_by_news_no', frm).val(list_by_news_no); // frm 폼에서 태그를 검색
...
<jsp:include page="/menu/top.jsp" flush='false' />
  <form name='frm' id='frm' method='post' action='./delete.do'>
    ...
    <input type='hidden' name='rurl' id='rurl' value='./list_by_news_no.do'>
  ...
  <DIV class='title_line'>
    ${newsVO.news_title } 첨부 파일
  </DIV>

  <ASIDE class='aside_left'>
    <A href='../newss/read.do?news_no=${newsVO.news_no }'>관련글</A> >
    <A href='./create.do?news_no=${newsVO.news_no }&news_no=${newsVO.news_no }'>첨부 파일 등록</A>
  </ASIDE>
    ...
    <table class="table table-striped" style='width: 100%;'>
      ...
      <tbody>
        <c:forEach var="News_attachfileVO" items="${list }">
          ...
          <c:set var="news_attachfile_name" value="${News_attachfileVO.news_attachfile_name.toLowerCase() }" />
          
          <tr> 
            <td style='vertical-align: middle; text-align: center;'>${News_attachfileVO.news_attachfile_no }</td>
            <td style='vertical-align: middle; text-align: center;'>
              <c:choose>
                <c:when test="${news_attachfile_name.endsWith('jpg') || news_attachfile_name.endsWith('png') || news_attachfile_name.endsWith('gif')}">
                  <IMG src="./storage/${News_attachfileVO.news_attachfile_thumb }" style="width: 120px;"> 
                </c:when>
                <c:otherwise> <!-- 이미지가 아닌 일반 파일 -->
                  ${News_attachfileVO.fname}
                </c:otherwise>
              </c:choose>
            </td> 
            <td style='text-align: center; vertical-align: middle;'>${News_attachfileVO.news_attachfile_name}</td>
            ...
            <td style='text-align: center; vertical-align: middle;'>${News_attachfileVO.news_attachfile_rdate.substring(0, 10)}</td>
