...
<script type="text/javascript">
  ...
  function count_by_news_no() {
    ...
    $.ajax({
      url: '../attachfile/count_by_news_no.do', // spring execute
      type: 'get',  // post
      ...
      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
        ...
        if (rdata.cnt > 0) {
          ...
          msg += "<button type='button' onclick='delete_by_news_no()' class='btn btn-danger'>첨부 파일 삭제</button>";
  ...
  function delete_by_news_no() {
    ...
    $.ajax({
      url: '../attachfile/delete_by_news_no.do', // spring execute
      type: 'post',  // get, post
      cache: false, // 응답 결과 임시 저장 취소
      async: true,  // true: 비동기 통신
      dataType: 'json', // 응답 형식: json, html, xml...
...
<jsp:include page="/menu/top.jsp" flush='false' />
  ...
  <ASIDE class="aside_left">
    ...
    <A href="../news/list_by_newsgrp_no_grid1.do?newsgrp_no=${newsgrpVO.newsgrp_no }">${newsgrpVO.newsgrp_head }</A> > [${newsVO.news_head }] 글 수정
  </ASIDE>
  <ASIDE class="aside_right">
    <A href='./list.do?newsgrp_no=${param.newsgrp_no }'>목록</A>
  ...
  <FORM name='frm' method='POST' action='./delete.do'>
      <input type='hidden' name='news_no' value='${param.news_no}'>
      <input type="hidden" name="newsgrp_no" value="${param.newsgrp_no }">
      ...
      <div class="form-group">   
        <div class="col-md-12" style='text-align: center; margin: 10px auto;'>
          삭제 되는글: ${newsVO.news_head }<br><br>
          삭제하시겠습니까? 삭제하시면 복구 할 수 없습니다.<br><br>
          
        <div class="form-group">   
        <div class="col-md-12" style='text-align: center; margin: 10px auto;'>
          <input type='password' class="form-control" name='news_passwd'  value='' placeholder="패스워드" style='width: 20%; margin: 0px auto;'>
        </div>
      </div>
